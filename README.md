...
A Flask-based prototype for practicing technical interviews with a voice-first workflow. Upload a resume and hold a live conversation with DeepSeek – no typing required.
...
- Session-aware DeepSeek prompts that incorporate resume context (resume + history kept server-side per session).
...
2. Provide your credentials in a `.env` file (or export them in your shell):
   ...
   echo "DEEPSEEK_PHASE_MODEL=deepseek-chat" >> .env
   ```
...
3. Run the development server on the requested port:
   ...
   or use the helper script (it ensures dependencies are installed and requires `DEEPSEEK_API_KEY` to be set):
...
4. Open `http://localhost:1111` and:
   ...
   - Speak naturally; DeepSeek will respond out loud.
...
- DeepSeek replies stream back with Kokoro-generated audio that plays immediately; barge-in is supported, so speaking will interrupt the model mid-sentence.
- The backend keeps the full conversation context and resume copy so each DeepSeek response remains grounded.

## Next Ideas

- Add WebRTC support for full-duplex streaming with a realtime model provider.
